    ...
    <section class="section">
      ...
      <template v-if="!uploading">
        ...
        <div class="mb-4" v-else>
          Upload completed!
          <button class="button is-info is-small ml-2" @click="removeFile">
            Cancel
          </button>
        </div>
      </template>
      ...
      <div class="field">
        ...
        <div class="control">
          <input
            class="input"
            type="number"
            placeholder="Royalty Percentage (e.g. 2)"
            v-model="royalty"
          />
        </div>
      </div>
      ...
      <div
        class="mt-5 has-text-centered has-text-weight-bold"
        v-if="message !== ''"
      >
        <span>{{ message }}</span>
      </div>
...
export default class Create extends Vue {
  ...
  async createNft(): Promise<void> {
    if (!Number.isInteger(this.royalty)) {
      console.log("this.royalty:" + this.royalty);
